...
exports.updateMe = catchAsync(async (req, res, next) => {
  ...
  if (req.body.password || req.body.passwordconfirm) {
    return next(
      new AppError(
        'This is not for password update. Please use URL:/updateMyPassword',
        400
      )
    );
  }
  //2)Filtered out unwanted fields names that are not allowed to be updated
  const filteredBody = filterObject(req.body, 'name', 'email','address','eduction','job','phoneNo','website',
